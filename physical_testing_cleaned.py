...
class SafetyMonitor:
    def __init__(self, position_limits: Dict[str, Tuple[float, float]], max_velocity=100.0, max_delta=50.0):
        ...
        for step in range(max_steps):
            ...
            try:
                ...
                if execution_success:
                    ...
                    if self.use_absolute:
                        print(f"Step {step+1:3d}: ✓ Raw Action = {raw_action[:3]}")
                        print(f"             Absolute Target = {absolute_action[:3]}")
                        print(f"             Movement Delta = {actual_deltas}")
                    else:
                        print(f"Step {step+1:3d}: ✓ Raw Delta = {raw_action[:3]}")
                        print(f"             Scaled Delta = {actual_deltas}")
                        print(f"             Target = {absolute_action[:3]}")
                else:
                    print(f"Step {step+1:3d}: ✗ Failed to execute action")
